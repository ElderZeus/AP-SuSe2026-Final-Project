...
This project was developed collaboratively rather than split into fixed
roles
...
1. Start the provided TCP server (from Exercise 5) on the machine/port you want to stream from.
    ## Running the TCP server (required before connecting)

    This application is a **client only** — it does not start the server itself.
    You need two terminals running at the same time:

    1. **Terminal 1 — start the server** (provided separately, from Exercise 5):

           cd path/to/TCP_Server
           python main.py

       Leave this terminal running. It should print a message confirming it is
       listening for connections.

    2. **Terminal 2 — start this application**, with its virtual environment
       activated:

           cd path/to/final_project
           python main.py

    3. Note the port the server is listening on (see its startup output or the
       `PORT` constant near the top of its `main.py`) — you will need to enter
       this exact port number in the app's Port field.
...
- **"Could not connect to server"** — check that the TCP server is running
  *and* that the port field matches the server's port exactly.
- **PowerShell activation error on Windows** — see the execution-policy note
  above.
- **`ModuleNotFoundError`** — confirm your virtual environment is activated
  (prompt should show `(.venv)`) before running `pip install` or `python main.py`.
